window.addEventListener('DOMContentLoaded', function () {
    ...
    const sendForm = (form) => {
        ...
        form.addEventListener('submit', (e) => {
            ...
            postData(body)
                .then( (status) => { statusMessage.src = successMessage; console.log(status); })
                .catch( (error) => { statusMessage.src = errorMessage; console.log(error); });
        });
        ...
        const postData = (body) => {
            return new Promise((resolve, reject) => {
                ...
                request.addEventListener('readystatechange', ()=> {
                    ...
                    if (request.status === 200) { resolve(request.status); clearForm(); }
                    else { reject(request.status); clearForm(); }
                });
